...
class EventDrivenBacktestEngine(BaseBacktestEngine):
    ...
    def _handle_signal(self, event: Dict[str, Any]) -> None:
        ...
        # Convert backtest portfolio representation to PortfolioState dataclass expected by RiskEngine
        p_state = PortfolioState(
            current_equity=self.portfolio.cash, # simple approximation
            open_positions=self.portfolio.positions.copy(),
            daily_pnl=self.portfolio.realized_pnl,
            weekly_pnl=self.portfolio.realized_pnl,
            monthly_pnl=self.portfolio.realized_pnl,
            win_rate=0.5,
            win_loss_ratio=1.0,
